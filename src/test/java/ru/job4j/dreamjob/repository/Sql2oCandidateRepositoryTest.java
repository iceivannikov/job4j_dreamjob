...
class Sql2oCandidateRepositoryTest {
    ...
    public void whenSaveThenGetSame() {
        ...
        var candidate = sql2oCandidateRepository.save(new Candidate(0, "title", "description", creationDate, file.getId()));
        var saveCandidate = sql2oCandidateRepository.findById(candidate.getId()).get();
    ...
    public void whenSaveSeveralThenGetAll() {
        var creationDate = now().truncatedTo(ChronoUnit.MINUTES);
        var candidate1 = sql2oCandidateRepository.save(new Candidate(0, "title1", "description1", creationDate, file.getId()));
        var candidate2 = sql2oCandidateRepository.save(new Candidate(0, "title2", "description2", creationDate, file.getId()));
        var candidate3 = sql2oCandidateRepository.save(new Candidate(0, "title3", "description3", creationDate, file.getId()));
    ...
    public void whenUpdateThenGetUpdated() {
        ...
        var updatedCandidate = new Candidate(
                candidate.getId(), "new title", "new description", creationDate.plusDays(1), file.getId());
    ...
    public void whenUpdateUnExistingCandidateThenGetFalse() {
        var creationDate = now().truncatedTo(ChronoUnit.MINUTES);
        var candidate = new Candidate(0, "title", "description", creationDate, file.getId());
